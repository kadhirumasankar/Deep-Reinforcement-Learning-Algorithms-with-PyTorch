...
class SAC(Base_Agent):
    ...
    def learn(self):
        ...
        self.tensorboard_writer.add_scalar(
            "qf1_loss", qf1_loss.item(), self.episode_number + 1
        )
        self.tensorboard_writer.add_scalar(
            "qf2_loss", qf2_loss.item(), self.episode_number + 1
        )
        self.tensorboard_writer.add_scalar(
            "policy_loss", policy_loss.item(), self.episode_number + 1
        )
        self.tensorboard_writer.add_scalar(
            "alpha_loss", alpha_loss.item(), self.episode_number + 1
        )
    ...
    def print_summary_of_latest_evaluation_episode(self):
        ...
        print("----------------------------")
        print(
            f"Episode {self.episode_number + 1} evaluation score {self.total_episode_score_so_far.item()}"
        )
        print("----------------------------")
        if (
            self.episode_number % TRAINING_EPISODES_PER_EVAL_EPISODE == 0
            ...
            with open(
                f"{self.model_dir}/memory_{len(self.game_full_episode_scores)}.pkl",
                "wb",
            ) as f:
