    ...
    def compute_critic_values_for_next_states(self, next_states):
        ...
        with torch.no_grad():
            actions_next = self.actor_target(
                torch.reshape(next_states, [-1, self.state_size])
            )
            ...
            critic_targets_next = torch.min(
                torch.cat((critic_targets_next_1, critic_targets_next_2), 1), dim=1
            )[0].unsqueeze(-1)
