...
class SAC_Discrete(SAC):
    ...
    def __init__(self, config):
        ...
        ), "Action types must be discrete. Use SAC instead for continuous actions"
        assert (
            self.config.hyperparameters["Actor"]["final_layer_activation"] == "Softmax"
        ), "Final actor layer must be softmax"
        self.hyperparameters = config.hyperparameters
        if self.config.load_model:
            self.critic_local = torch.load(
                f"{self.config.model_dir}/critic_local_{self.config.load_model_episode}.pt"
            )
            self.critic_local_2 = torch.load(
                f"{self.config.model_dir}/critic_local_2_{self.config.load_model_episode}.pt"
            )
            self.critic_target = torch.load(
                f"{self.config.model_dir}/critic_target_{self.config.load_model_episode}.pt"
            )
            self.critic_target_2 = torch.load(
                f"{self.config.model_dir}/critic_target_2_{self.config.load_model_episode}.pt"
            )
        ...
        assert not self.hyperparameters[
            "add_extra_noise"
        ], "There is no add extra noise option for the discrete version of SAC at moment"
    ...
    def produce_action_and_action_info(self, state):
        ...
        action_probabilities = self.actor_local(
            torch.reshape(state, [-1, self.state_size])
        )
        ...
        return (
            action,
            (action_probabilities, log_action_probabilities),
            max_probability_action,
        )
    ...
    def calculate_critic_losses(
        ...
        with torch.no_grad():
            ...
            ), _ = self.produce_action_and_action_info(next_state_batch)
            qf1_next_target = self.critic_target(
                torch.reshape(next_state_batch, [-1, self.state_size])
            )
        ...
        qf1 = self.critic_local(
            torch.reshape(state_batch, [-1, self.state_size])
        ).gather(1, action_batch.long())
        qf2 = self.critic_local_2(
            torch.reshape(state_batch, [-1, self.state_size])
        ).gather(1, action_batch.long())
    ...
    def calculate_actor_loss(self, state_batch):
        ...
        action, (
            action_probabilities,
            log_action_probabilities,
        ), _ = self.produce_action_and_action_info(state_batch)
