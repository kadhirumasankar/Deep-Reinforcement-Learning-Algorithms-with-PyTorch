...
class DQN(Base_Agent):
    ...
    def pick_action(self, state=None):
        ...
        with torch.no_grad():
            action_values = self.q_network_local(torch.reshape(state, [-1, self.state_size]))
    ...
    def compute_q_values_for_next_states(self, next_states):
        ...
        Q_targets_next = self.q_network_local(torch.reshape(next_states, [-1, self.state_size])).detach().max(1)[0].unsqueeze(1)
    ...
    def compute_expected_q_values(self, states, actions):
        ...
        Q_expected = self.q_network_local(torch.reshape(states, [-1, self.state_size])).gather(1, actions.long()) #must convert actions to long so can be used as index
        return Q_expected
