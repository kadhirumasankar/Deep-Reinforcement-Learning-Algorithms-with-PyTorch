...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--nepisodes", type=int, default=50000, help="Number of episodes"
    )
    ...
    config.file_to_save_data_results = (
        "results/data_and_graphs/Predator_Prey_Results_Data.pkl"
    )
    config.file_to_save_results_graph = (
        "results/data_and_graphs/Predator_Prey_Results_Graph.png"
    )
    ...
    if config.load_model and config.load_model_episode == 0:
        raise Exception(
            "config.load_model is set to True but no episode number provided"
        )
    elif not config.load_model and config.load_model_episode != 0:
        raise Exception(
            "config.load_model is set to False but load_model_episode is not set to 0"
        )
